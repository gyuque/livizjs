/* $Id: mincross.c,v 1.23 2011/02/04 18:36:12 erg Exp $ $Revision: 1.23 $ */
...
static int mincross(graph_t * g, int startpass, int endpass, int, xg_askShouldStop);
static void init_mincross(graph_t * g);
 ...
 * is called.
 */
void dot_mincross(graph_t * g, int doBalance, struct _DotStats* stats)
{
    int c, nc;
    char *s;
	...
	init_mccomp(g, c);
	nc += mincross(g, 0, 2, doBalance, stats->stopFunc);
    }
	...
	mark_lowclusters(g);
	ReMincross = TRUE;
	nc = mincross(g, 2, 2, doBalance, stats->stopFunc);
#ifdef DEBUG
	for (c = 1; c <= GD_n_cluster(g); c++)
	    check_vlists(GD_clust(g)[c]);
#endif
...
static int mincross_clust(graph_t * par, graph_t * g, int doBalance)
{
    ...
    flat_breakcycles(g);
    flat_reorder(g);
    nc = mincross(g, 2, 2, doBalance, NULL);
...
static int mincross(graph_t * g, int startpass, int endpass, int doBalance, xg_askShouldStop stopFunc)
{
	...
	if(stopFunc && stopFunc(PROGRESS_BEFORE_MX)) {
		return best_cross;
	}
	...
	if (pass <= 1) {
	    ...
	    flat_reorder(g);
			if (stopFunc && stopFunc(PROGRESS_WHILE_MX)) {
				return best_cross;
			}
