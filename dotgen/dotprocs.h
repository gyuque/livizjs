...
#include "aspect.h"
	#define PROGRESS_UNKNOWN    0
	#define PROGRESS_AFTER_RANK 1
	#define PROGRESS_BEFORE_MX  2
	#define PROGRESS_WHILE_MX   3
	#define PROGRESS_AFTER_MX   4
	#define PROGRESS_AFTER_POSITION  6
	#define PROGRESS_LAYOUT_FINISH   9

    typedef int (*xg_askShouldStop)(int currentProgress);
    typedef struct _DotStats {
		int iterCount;
		int stopped;
		xg_askShouldStop stopFunc;
    } DotStats;
    ...
    extern void dot_layout_intl(Agraph_t * g, struct _DotStats*);
...
#   define extern __EXPORT__
    ...
    extern void dot_mincross(Agraph_t *, int, struct _DotStats*);
    extern void dot_position(Agraph_t *, aspect_t*, struct _DotStats*);
    extern void dot_rank(Agraph_t *, aspect_t*, struct _DotStats* stats);
