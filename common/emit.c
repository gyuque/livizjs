/* $Id: emit.c,v 1.323 2011/02/21 23:20:51 erg Exp $ $Revision: 1.323 $ */
...
void emit_firstlayer(GVJ_t *job) { return firstlayer(job); }
...
void init_job_pagination(GVJ_t * job, graph_t *g)
{
    GVC_t *gvc = job->gvc;
...
void emit_firstpage(GVJ_t *job){ firstpage(job); }
...
void emit_setup_page(GVJ_t * job, graph_t * g) { setup_page(job, g); }
...
/* static */
void init_job_viewport(GVJ_t * job, graph_t * g)
{
    GVC_t *gvc = job->gvc;
